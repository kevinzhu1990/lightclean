...
- 首次安装可免费使用全部功能30天
- 卖家只需发送一张兑换码；客户在“授权与套餐”页面输入后，首次联网激活会自动绑定当前电脑
- 授权服务拒绝同一兑换码在另一台电脑同时激活；换机必须联系卖家人工审核，客户不能自行解绑
- 激活时向授权服务发送兑换码和匿名设备标识；服务端只保存兑换码摘要，客户端用公钥验证签名凭证
- 激活成功后可离线使用 14 天；到期需联网刷新授权。人工换机后，旧电脑的离线凭证在剩余有效期内仍可能使用；旧版双码离线签发流程仅作为兼容方式保留
- 在线授权服务见 `license-cloud`；签名私钥与兑换码数据库不应进入公开仓库
